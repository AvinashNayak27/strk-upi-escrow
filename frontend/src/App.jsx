    ...
    const fetchPrice = async () => {
      try {
        const res = await fetch(
          "https://api.coingecko.com/api/v3/simple/price?ids=starknet&vs_currencies=inr"
        );
        const data = await res.json();
        setStrkPrice(data.starknet?.inr ?? null);
      } catch {
        /* ignore */
      }
    };
    ...
    <div className="app">
      <header className="topbar">
        <div className="brand">
          ...
          <div>
            <h1 className="brand-name">STRK / INR</h1>
      ...
      {!wallet ? (
        <main className="hero">
          <div className="hero-content">
            <h2>Buy and sell STRK with INR</h2>
            <p>
              ...
              <form className="sell-form" onSubmit={createSellOrder}>
                ...
                {sellAmount && pricePerStrk && (
                  <div className="sell-summary">
                    ...
                    <strong>
                      ₹{(parseFloat(sellAmount || 0) * parseFloat(pricePerStrk || 0)).toFixed(2)} INR
                    </strong>
                    ...
                    return (
                      <div key={o.depositId} className={`order-card ${hasActiveIntent ? "order-locked" : ""}`}>
                        ...
                        <div className="order-meta">
                          ...
                          <div className="meta-row">
                            <span className="meta-label">Total INR</span>
                            <span className="meta-value meta-total">
                              ₹{((Number(o.amountStrk) / 1e18) * Number(o.pricePerStrk)).toFixed(2)}
                            </span>
                          </div>
